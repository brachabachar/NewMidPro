...
export class NoteComponent implements OnInit {
  ...
  AllNote() {
    this.noteService.GetNotesNoRead(this.user.Id).subscribe((notes) => {
      ...
      this.noteList.forEach(x => this.allNote.List.push(new MapList(x.Id, "הודעה מס:" , x.Id.toString())));
    });
  ...
  noReadNote() {
    this.noteService.GetNotesNoRead(this.user.Id).subscribe((notes) => {
      this.noteList = JSON.parse(notes.toString());
      this.allNote.Title = "הודעות חדשות שלי";
      this.noteList.forEach(x => this.allNote.List.push(new MapList(x.Id, "הודעה מספר:" + x.Id.toString()+",", x.Description.slice(0,20))));
    });
  }
  ...
  userNote(userId: number) {
    this.noteService.GetNotesByUserId(userId).subscribe((notes) => {
      this.noteList = JSON.parse(notes.toString());
      this.allNote.Title = "";
      this.noteList.forEach(x => this.allNote.List.push(new MapList(x.Id, " " , x.Id.toString())));
    });
  }
  GetNotesByUserIdCreat(userId: number) {
    this.noteService.GetNotesByUserIdCreat(userId).subscribe((notes) => {
      this.noteList = JSON.parse(notes.toString());
      this.allNote.Title = "";
      this.noteList.forEach(x => this.allNote.List.push(new MapList(x.Id, " :מספר קורקינט" , x.Id.toString())));
    });
  }
